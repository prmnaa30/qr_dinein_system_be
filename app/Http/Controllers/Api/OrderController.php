...
class OrderController extends Controller
{
    ...
    #[Endpoint('Tracking Status Pesanan', 'Melihat status pesanan setelah/saat pembayaran melalui Midtrans')]
    #[Response(content: '{"data": {"order_id": 1,"customer_name": "John Doe","table_number": "A1","status": "pending","payment_status": "paid","snap_token": "xxxx-xxx-xxxx","ui_step": 1,"ui_description": "Pesanan menunggu konfirmasi dapur","items": [{"name": "Nasi Goreng","qty": 1}]}}', status: 200)]
    ...
    {
        $order = $this->orderService->getOrderForTracking($id);

        if (!$order) {
            return response()->json(['message' => 'Order tidak ditemukan!'], 404);
        }

        if ($request->has('table_id') && !$this->orderService->validateTableAccess($order, $request->table_id)) {
            return response()->json(['message' => 'Akses ditolak. Nomor meja tidak sesuai.'], 403);
        }

        $trackingInfo = $this->orderService->getOrderTrackingInfo($order);
